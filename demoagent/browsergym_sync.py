...
def extract_axtree(page: Page) -> Dict[str, Any]:
    """
     ...
     */
    ([parent_bid, bid_attr_name, tags_to_mark]) => {
        ...
        // Synchronous version: We can't use async/await or Promises directly, so we use a timeout instead
        setTimeout(() => {
            // disconnect intersection observer after timeout - may not have visited all elements
            intersection_observer.disconnect();
        }, 1000); // Wait 1000ms for visibility calculations
    ...
    page.evaluate(js_frame_mark_elements, ["", BROWSERGYM_ID_ATTRIBUTE, "standard_html"])
    
    # Wait a bit for the intersection observer to do its job (since we can't use async/await in the browser)
    ...
    for frame_id in frame_ids:
        ax_tree = cdp.send("Accessibility.getFullAXTree", {"frameId": frame_id})
    ...
    for node in merged_axtree["nodes"]:
        if node.get("role", {}).get("value") == "Iframe":
            ...
            if "backendDOMNodeId" in node:
                ...
                frame_info = cdp.send("DOM.describeNode", {"backendNodeId": node["backendDOMNodeId"]})
    ...
    # Clean up by detaching CDP session
    cdp.detach()
    
    return merged_axtree


def extract_dom_extra_properties(page: Page, merged_axtree: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
    """
    ...
    extra_properties = page.evaluate(js_get_extra_props)
    return extra_properties


def get_observation(page: Page) -> Dict[str, Any]:
    """
    ...
    merged_axtree = extract_axtree(page)
    
    # Extract extra properties for DOM elements
    extra_properties = extract_dom_extra_properties(page, merged_axtree)
...
def noop(page: Page, wait_ms: float = 1000):
    """Wait for a specified amount of time."""
    page.wait_for_timeout(wait_ms)


def scroll(page: Page, delta_x: float, delta_y: float):
    """Scroll the page by the specified delta."""
    page.evaluate(f"window.scrollBy({delta_x}, {delta_y})")


def fill(page: Page, bid: str, value: str):
    """Fill an input element with text."""
    element = page.locator(f"[{BROWSERGYM_ID_ATTRIBUTE}='{bid}']")
    element.fill(value)


def select_option(page: Page, bid: str, options: Union[str, List[str]]):
    """Select option(s) in a select element."""
    element = page.locator(f"[{BROWSERGYM_ID_ATTRIBUTE}='{bid}']")
    if isinstance(options, str):
        element.select_option(value=options)
    else:
        element.select_option(values=options)


def click(page: Page, bid: str, button: Literal['left', 'middle', 'right'] = 'left', 
           modifiers: List[Literal['Alt', 'Control', 'Meta', 'Shift']] = None):
    """Click on an element."""
    if modifiers is None:
        modifiers = []
    element = page.locator(f"[{BROWSERGYM_ID_ATTRIBUTE}='{bid}']")
    element.click(button=button, modifiers=modifiers)


def dblclick(page: Page, bid: str, button: Literal['left', 'middle', 'right'] = 'left',
              modifiers: List[Literal['Alt', 'Control', 'Meta', 'Shift']] = None):
    """Double-click on an element."""
    if modifiers is None:
        modifiers = []
    element = page.locator(f"[{BROWSERGYM_ID_ATTRIBUTE}='{bid}']")
    element.dblclick(button=button, modifiers=modifiers)


def hover(page: Page, bid: str):
    """Hover over an element."""
    element = page.locator(f"[{BROWSERGYM_ID_ATTRIBUTE}='{bid}']")
    element.hover()


def press(page: Page, bid: str, key_comb: str):
    """Press a key combination on an element."""
    element = page.locator(f"[{BROWSERGYM_ID_ATTRIBUTE}='{bid}']")
    element.press(key_comb)


def focus(page: Page, bid: str):
    """Focus on an element."""
    element = page.locator(f"[{BROWSERGYM_ID_ATTRIBUTE}='{bid}']")
    element.focus()


def clear(page: Page, bid: str):
    """Clear an input element."""
    element = page.locator(f"[{BROWSERGYM_ID_ATTRIBUTE}='{bid}']")
    element.fill("")


def drag_and_drop(page: Page, from_bid: str, to_bid: str):
...
def upload_file(page: Page, bid: str, file_paths: Union[str, List[str]]):
...
def tab_close(page: Page):
    """Close the current tab."""
    page.close()


def tab_focus(page: Page, index: int):
...
def new_tab(page: Page, url: Optional[str] = None):
...
def send_msg_to_user(page: Page, text: str):
...
def report_infeasible(page: Page, reason: str):
...
def execute_action(page: Page, action: str) -> Dict[str, Any]:
    """
...
class BrowserController:
    ...
    Uses the synchronous Playwright API.
    """
...
def main():
    parser = argparse.ArgumentParser(description='Browser automation with Playwright in BrowserGym style (synchronous)')
    ...
    with BrowserController() as controller:
        ...
        if args.action:
            ...
            result = controller.execute_action(args.action)

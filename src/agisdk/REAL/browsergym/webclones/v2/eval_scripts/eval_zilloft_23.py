        ...
        if k in ADDRESS_KEYS or lk in ADDRESS_KEYS or "address" in lk or "message" in lk:
            ...
            if isinstance(v, dict):
                ...
                st = (
                    v.get("state")
                    or v.get("stateCode")
                    or v.get("addressState")
                    or v.get("abbreviatedState")
                )
                if isinstance(st, str) and (
                    st.strip().upper() == "CA" or "california" in st.lower()
                ):
                    return "CA"
            ...
            for k, v in cur.items():
                ...
                if k in STATE_KEYS or lk in STATE_KEYS:
                    if isinstance(v, str) and (
                        v.strip().upper() == "CA" or "california" in v.lower()
                    ):
                        found_ca_loc = True
                if (
                    k in ADDRESS_KEYS

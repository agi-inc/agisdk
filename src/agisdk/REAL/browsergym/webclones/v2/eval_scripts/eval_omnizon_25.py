...
def extract_item_candidates(root):
    ...
    for node in iter_nodes(root):
        if isinstance(node, dict):
            ...
            has_apron = any(is_apron(v) for _, v in text_fields) or (
                combined_text and is_apron(combined_text)
            )
            has_pot = any(is_cooking_pot(v) for _, v in text_fields) or (
                combined_text and is_cooking_pot(combined_text)
            )
...
def main():
    ...
    if (
        (order_confirmed or in_cart_context)
        and has_apron_text
        and has_pot_text
        and not suspicious_high_qty

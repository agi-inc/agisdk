...
def find_wallet(node):
    ...
    if isinstance(node, dict):
        ...
        if 'balance' in node and 'transactions' in node and isinstance(node.get('transactions'), list):
...
def main():
    ...
    for t in transactions:
        ...
        ttype = t.get('type')
        if desc == 'Wallet Top-up' and ttype == 'credit' and amt is not None and abs(amt - 100.0) < 1e-9:
            count_100_credit += 1
    ...
    success = (
        balance is not None and abs(balance - 125.5) < 1e-9 and
        count_100_credit == 1 and
        has_credit_card
    )

...
def main():
    ...
    spam_ids = {str(eid) for eid, ch in email_updates.items() if isinstance(ch, dict) and ch.get('spam') is True}
    spam_on_high_id = any(sid in {"51", "52", "53", "54", "55", "56"} for sid in spam_ids)

    if email_updates and not trashed_ids and len(updated_list) == 0 and not mention_trash and spam_on_high_id:
        for itm in deleted_list:

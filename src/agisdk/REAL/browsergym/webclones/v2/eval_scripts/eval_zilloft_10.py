...
def has_valid_tour_request(container):
    ...
    for _, entry in container.items():
        ...
        if isinstance(rtd, dict):
            ...
            if (isinstance(msg, str) and msg.strip()) or (isinstance(options, list) and len(options) > 0):
    ...
    def search_filters(obj):
        if isinstance(obj, dict):
            for k, v in obj.items():
                ...
                if any(term in k_lower for term in ['maxprice', 'max_price', 'pricemax', 'price_max', 'upperprice']):
                ...
                if any(term in k_lower for term in ['bedroom', 'bed', 'minbedroom', 'min_bedroom', 'bedrooms_min', 'minbeds']):

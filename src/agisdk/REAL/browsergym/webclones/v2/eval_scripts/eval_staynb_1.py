...
def check_search_criteria(search):
    """Check if search matches Austria, dates Aug 1-5, and 3 guests."""
    if not isinstance(search, dict):
        return False
    
    # Check destination is Austria
    destination = search.get('appliedDestination', '')
    if normalize_text(destination) != normalize_text('Austria'):
        return False
    ...
    if start_date != '2024-08-01' or end_date != '2024-08-05':
        return False
    
    # Check guest count totals 3

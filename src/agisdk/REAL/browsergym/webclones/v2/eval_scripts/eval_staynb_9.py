...
def canonicalize_amenity(raw):
    ...
    if 'internet' in s_norm and 'wireless' in s:
        return 'wifi'
    # AC / Air conditioning
    if any(tok in s_norm for tok in ['ac', 'airconditioning', 'airconditioner', 'aircondition', 'aircon']):
        return 'airconditioning'
    if 'air' in s and 'condition' in s:
        return 'airconditioning'
    # Free parking
    if ('parking' in s_norm and 'free' in s) or any(tok in s_norm for tok in ['freeparking', 'freeonpremisesparking', 'freeonpremiseparking', 'freeonstreetparking', 'freeon-siteparking']):
        return 'freeparking'
    # Pool
...
def is_tonight_dates(search):
    ...
    if not isinstance(dates, dict):
        return False
    sd = dates.get('startDate'); ed = dates.get('endDate')
...
def main():
    ...
    if provided:
        ...
        success = nearby_ok and dates_ok and no_booking and has_required and no_extra_amenities and no_other_filters
    else:

...
def near(text, w1, w2, max_span=80):
    pattern = re.compile(
        r"\b" + re.escape(w1) + r"\b.{0," + str(max_span) + r"}?\b" + re.escape(w2) + r"\b"
    )
    if pattern.search(text):
        return True
    pattern2 = re.compile(
        r"\b" + re.escape(w2) + r"\b.{0," + str(max_span) + r"}?\b" + re.escape(w1) + r"\b"
    )
...
def is_silverware_set_text(blob):
    ...
    if any(near(blob, c, "set", 80) for c in CATEGORY_WORDS):
        return True
    if re.search(
        r"\b(\d{1,3})\s*[- ]?(piece|pcs)\b.{0,80}\b(silver\s*ware|silverware|flatware|cutlery)\b",
        blob,
    ):
        return True
